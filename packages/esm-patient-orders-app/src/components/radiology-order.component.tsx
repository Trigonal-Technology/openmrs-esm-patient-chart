...
import { ViewIcon, formatDate } from '@openmrs/esm-framework';
import { type RadiologyOrder } from '../types/radiologyOrders';
import DynamicContent from './DynamicContent';
import AnalysisPopup from './AnalysisPopup';
import ReactDOM from 'react-dom';
import { setRandomImage } from '../utils/imageUtils';
import { t } from 'i18next';
import { DataAnalytics } from '@carbon/react/icons';
...
const RadiologyOrderComponent: React.FC<RadiologyOrderProps> = ({ radiologyOrder, patientId }) => {
  ...
  //   }
  // };
  const handleViewClick = async (event: React.MouseEvent<HTMLButtonElement>, patientId: string): Promise<void> => {
    ...
    try {
      ...
      // View action failed; caller may surface UI feedback later.
    }
  };
  ...
  return (
    <div className={styles.radiologyController}>
      ...
      <IconButton
        ...
        kind="ghost"
        onClick={(event) => {
          handleViewClick(event, (patientId = '10001NG'));
        }}
      >
        <ViewIcon />
      </IconButton>
