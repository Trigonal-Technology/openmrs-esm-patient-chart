...
export const transformOrderSetMember = (member: any, visit: Visit, providerUuid: string, config: ConfigObject) => {
  ...
  if (memberType === 'DRUG') {
    const drugOrder = {
      action: 'NEW',
      asNeeded: false,
      concept: '0cd880aa-b422-466d-a9b2-951173624acd',
      drug: member.conceptUuid,
      dosage: member.dose,
      ...
      durationUnit: member.durationUnitUuid ? { valueCoded: member.durationUnitUuid, value: member.durationUnitDisplay } : null,
  ...
  const category = memberType === 'LAB' ? 'Test' : memberType === 'RADIOLOGY' ? 'Radiology/Imaging Procedure' : 'Procedure';
