        ...
        <div className={styles.clipTextWithEllipsis}>
          ...
          <div className={styles.dosageInfo}>
            <span className={styles.label01}>
              {t('dose', 'DOSE').toUpperCase()} {row.dose ?? '--'} {doseUnit?.value ?? ''} &mdash;{' '}
              {route?.value ?? '--'} &mdash; {freq ? (freq as { value?: string }).value : '--'} &mdash;{' '}
              {t('quantity', 'QUANTITY').toUpperCase()} {row.quantity ?? '--'} {doseUnit?.value ?? ''}
            </span>
          </div>
        </div>
        <IconButton
